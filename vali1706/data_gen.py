...
caseName = ['6F2']
txt_filepath_root = r"D:\python_work\tsai-main-v39\main\vali1706\data179\csvData"
arguments = ['RotorSpd', 'RotorAng', 'BusVolMag', 'BusVolAng', 'LinCurMag', 'LinCurAng']  #['LinCurAng']
for i_case in range(0,1):#14
    caseNamei = caseName[i_case]
    for i_arg in range(0,len(arguments)):
        #load data
        csv_name = txt_filepath_root + '\\case'+f'{caseNamei}' + f'{arguments[i_arg]}' + ".csv"
        df_load = pd.read_csv(csv_name, index_col=0)

        # #截取2700行 删掉time列
        # df_cut = df_load.iloc[0:2700,:].drop(labels=["'Time"], axis=1)
        #截取后1200行 删掉time列 并保留time用作时间轴
        df_cut = df_load.iloc[1:1201, :]
        ...
        if caseNamei == '6F2':
            ...
            target_np = np.where(df_T.index.str.contains("NORTH G3    20.0"), 1, 0)
        # elif  i_case == 2:
        ...
        if i_arg == 5:
            ...
            power_P = df_T.copy()
            power_Q = df_T.copy()
            WD = pd.DataFrame(0, index=df_T.index, columns=range(1,1201))
            # ab
            # eps = 0.05
            # fo = 0.6
            # fh = (1 + eps) * fo
            # fl = (1 - eps) * fo
            # b, a = signal.butter(4, [fl, fh], 'bandpass', fs=30)
            ...
            eps = 0.05
            fo = 0.6/15
            N, Wn = signal.buttord([(1 - eps) * fo, (1 + eps) * fo], [(1 - 2*eps) * fo, (1 + 2*eps) * fo], gpass=5, gstop=15)
            ...
            for i_i in range(0,len(LinCurMag_title)):
                ...
                for i_u in range(0,len(BusVolMag_title)):
                    ...
                    if match_u[0] == match_i[0]:   #电流流出节点match_i[0],电流流入节点match_i[2]
                        ...
                        V_xing = d_Vi + V_avr*np.ones(1200)#np.expand_dims(V_avr,1).repeat(1200,axis=1)
                        for n_t in range(0, 1199):
            ...
            csv_name = save_filepath + r"\case" + f'{caseNamei}' + "WD0_6nor" + ".csv"
            WD.to_csv(csv_name)

...
num_1 = 0
num_k = 2e3
txt1 = "ad_mr_rccv1-2e3k.txt"
txtmat = "ad_mr_rccv1-2e3k-mat.txt"
...
for i in range(1, 14):
    ...
    #似乎是整形
    n_instances, n_timepoints = X_resampled.shape
    X_resampled = np.reshape(X_resampled.values, (n_instances, 1, n_timepoints))
    ...
    # 适应训练数据并转换它
    minirocket.fit(X_resampled)
    X_train_transform = minirocket.transform(X_resampled)

    # 使用转换后的特征训练线性分类器
    # 1使用 RidgeClassifierCV 并且加入了计算出的类别权重
    classifier = RidgeClassifierCV(class_weight=class_weight_dict)
    classifier.fit(X_train_transform, y_resampled)
